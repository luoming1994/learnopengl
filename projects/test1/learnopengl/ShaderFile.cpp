...
void Shader::Set4FloatVector(const std::string &name, GLfloat *vs) const
{
	GLint location = glGetUniformLocation(mShaderId, name.c_str());
	GLsizei count = 4;
	glUniform4fv(location, count, vs);
}

void Shader::SetMat4Float(const std::string &name, glm::mat4 &transform) const
{
	GLint location = glGetUniformLocation(mShaderId, name.c_str());
...
void Shader::SetTextureIndex(const std::string &name,int index) const
{
	if ( index < 0 || index > (GL_TEXTURE31 - GL_TEXTURE0))
		std::cout<< "SetTextureIndex index is out of range ;0~" << (GL_TEXTURE31 - GL_TEXTURE0) << std::endl;
	glUniform1i(glGetUniformLocation(mShaderId, name.c_str()), GL_TEXTURE0 + index);
}
